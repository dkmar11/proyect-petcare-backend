...
const { validateBookingInput, validateProviderCapability, requiresVaccine } = require("../domain/rules/booking.rules");
...
async function confirmPayment(id) {
  ...
  const paid = await prisma.booking.update({ where: { id }, data: { paymentStatus: "PAID" }, include });
  await notifications.create(booking.userId, booking.id, "PAYMENT_CONFIRMED", "Pago confirmado", "Tu pago online fue registrado correctamente.");
  return paid;
}
module.exports = { create, listByUser, updateStatus, confirmPayment };
