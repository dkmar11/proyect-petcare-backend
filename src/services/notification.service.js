...
const repository = require("../infrastructure/notifications/notification.repository");

const statusMessages = {
  CONFIRMED: ["Reserva confirmada", "Tu cita fue confirmada por el proveedor."],
  IN_PROGRESS: ["Servicio en curso", "El proveedor ya está atendiendo a tu mascota."],
...
async function sendReminders() {
  const from = new Date(); const until = new Date(from.getTime() + 24 * 60 * 60 * 1000);
  ...
  for (const booking of bookings) {
    ...
    if (!exists) { await create(booking.userId, booking.id, "APPOINTMENT_REMINDER", "Recordatorio de cita", `Mañana tienes ${booking.serviceType.toLowerCase()} para ${booking.pet.name}.`); sent += 1; }
  }
  return sent;
}
